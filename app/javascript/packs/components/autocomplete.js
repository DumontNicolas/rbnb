...
const autoComplete = () => {
  search.addEventListener("keyup", (event) => {
    ...
    fetch(`${window.location.origin}/search?name=${event.target.value}`)
    ...
    .then((data) => {
      ...
      data.forEach(element => {
        ...
        results.insertAdjacentHTML('beforeend', `<li><a href="${window.location.origin}/games/${element.id}">${element.name}</a></li>`)
      });
    });
  });
};
